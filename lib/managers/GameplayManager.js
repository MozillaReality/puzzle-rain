...
function GameplayManager () {
  ...
  this.timeToEnd02StartElevating = 23;
  this.timeToEnd02CreditsStarted = 42.5;
...
GameplayManager.prototype.init = function () {
  ...
  Events.on('forceSadEnding', this.forceSadEnding.bind(this));
  Events.on('closerEnding', this.closerEnding.bind(this));
  // TODO remove this event, not useful with te new gameplay
...
GameplayManager.prototype.forceSadEnding = function (isSad) {
  // this.dispatchEnding();
  if (State.get('stage') === 'experience') {
    if (isSad) {
      State.add('endMode', 2);
