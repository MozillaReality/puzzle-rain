...
var isTrackpadPressed = false;
var lastTrackpadPressed = false;
...
function KeyManager () {
  ...
  console.log('Press "z" to press trigger with active hand');
  console.log('Press "x" to press trackpad with active hand');
}
...
function update () {
  ...
  if (lastTriggerPressed !== isTriggerPressed) {
    lastTriggerPressed = isTriggerPressed;
    // handL because is the Dummy one
    Events.emit('triggerKeyPressed', activeHand, isTriggerPressed);
  }
  if (lastTrackpadPressed !== isTrackpadPressed) {
    lastTrackpadPressed = isTrackpadPressed;
    // handL because is the Dummy one
    Events.emit('trackpadKeyPressed', activeHand, isTrackpadPressed);
  }
}
