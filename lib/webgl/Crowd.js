...
Crowd.prototype.addCreatures = function () {
  // TODO adjust position depending on room size
  var bouncers = new Bouncers(new THREE.Vector3(0.3, 0, -1.2));
  this.scene.add(bouncers);
  var bulrushes = new Bulrushes(new THREE.Vector3(-0.75, 0, -0.45));
  this.scene.add(bulrushes);
  var flyers = new Flyers(new THREE.Vector3(0.6, 0, 0.75));
  this.scene.add(flyers);
  var minerals = new Minerals(new THREE.Vector3(1, 0, -0.45));
  this.scene.add(minerals);
  var terrestrials = new Terrestrials(new THREE.Vector3(-0.45, 0, 0.60));
