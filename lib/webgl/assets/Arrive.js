...
function Arrive (obj) {
  ...
  THREE.Mesh.call(this, new THREE.SphereGeometry(0.05, 3, 2), new THREE.MeshBasicMaterial({ color: settings[this.obj.race + 'Color']}));
...
Arrive.prototype = Object.create(THREE.Mesh.prototype);
...
Arrive.prototype.isInfluencedByHands = function () {
  ...
  if (this.target0 !== undefined && this.target0.activeRace === this.obj.race) {
    influenced = true;
  }
  if (this.target1 !== undefined && this.target1.activeRace === this.obj.race) {
    influenced = true;
  }
  return influenced;
};
...
Arrive.prototype.moveByMode = function (mode) {
  switch (mode) {
    ...
    case 'awakeIdle':
      // NOW, DO NOTHING
      // TODO MAKE A SMOOTH MOVEMENT
      break;
    case 'awakeInfluenced':
...
Arrive.prototype.getTargetPosition = function () {
  ...
  var target = this.obj.speaker.position.clone().sub(this.obj.position);
  ...
  if (this.target0 !== undefined && this.target0.activeRace === this.obj.race && this.target1 !== undefined && this.target1.activeRace === this.obj.race) {
    // Mid point
