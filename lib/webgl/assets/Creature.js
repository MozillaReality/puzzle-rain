...
function Creature (race, index, myPos, myScale, minHeight, maxHeight) {
  ...
  this.maxRandEmissive = 0.5 + Math.random() * 0.2;
...
Creature.prototype.update = function (delta, time) {
  ...
  if (this.race.status === 'awake') {
    this.bodyMesh.material.emissiveIntensity = THREE.Math.mapLinear(this.track.averageAnalyser * this.race.awake.awakeLevel, 0, 100, 0, this.maxRandEmissive);
  }
};
...
Creature.prototype.addGlow = function () {
  var glowGeometry = new THREE.SphereGeometry(0.7, 16, 16);
