...
Creature.prototype.getPositionWithMagnet = function () {
  if (State.get('gamepadR') && State.get('gamepadL')) {
    ...
    var posToGamepad = getPointInBetweenByPerc(State.get('gamepadR').position, this.pos.clone().add(this.race.position), this.magnetR);
    if (this.magnetR > this.magnetL) {
      activeMagnet = this.magnetL;
      posToGamepad = getPointInBetweenByPerc(State.get('gamepadL').position, this.pos.clone().add(this.race.position), this.magnetL);
    }
    if (activeMagnet < 1) {
      posToGamepad = posToGamepad.clone().sub(this.race.position);
