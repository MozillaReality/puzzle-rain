...
function Creature (race, index, myPos, myScale, minHeight, maxHeight) {
  ...
  this.tweenToFunnyPosition = false;
  this.funnyYpos = 0.5;

  this.tweenToRainPosition = false;
  this.rainYpos = 0.9;
...
Creature.prototype.update = function (delta, time) {
  ...
  if (this.isRainStarted) {
    this.updateOnRaining(delta, time);
    return;
  }
  if (this.isFunnyCreature) {
    this.updateOnFunny(delta, time);
    return;
  }
  if (this.track) {
...
Creature.prototype.updateOnFunny = function (delta, time) {
  ...
  var userPos = new THREE.Vector3().setFromMatrixPosition(this.camera.matrixWorld).sub(this.position).sub(this.parent.position);
  this.body.lookAt(new THREE.Vector3(userPos.x, 0, userPos.z));
  if (this.tweenToFunnyPosition) {
    this.position.y = this.funnyYpos + (Math.sin((time * this.randExcited)) * 0.02);
  }
};

Creature.prototype.updateOnRaining = function (delta, time) {
  this.mouth.scale.y = 8;
  ...
  var userPos = new THREE.Vector3().setFromMatrixPosition(this.camera.matrixWorld).sub(this.position).sub(this.parent.position);
  this.body.lookAt(new THREE.Vector3(userPos.x, 0, userPos.z));
  if (this.tweenToRainPosition) {
    this.position.y = this.rainYpos + (Math.sin((time * this.randExcited)) * 0.02);
  }
};
...
Creature.prototype.addTrail = function () {
  ...
  this.trail = this.particleSystem.createTrail(
    200   ,
    g_trailParameters,
    new THREE.TextureLoader().load('textures/magicOver.png'));
...
Creature.prototype.stageChanged = function () {
  var self = this;
...
Creature.prototype.rainStarted = function () {
  this.isRainStarted = true;
  new TWEEN.Tween(this.position).to({
    y: this.rainYpos
  }, 6000)
    .onComplete(function () {
      self.tweenToRainPosition = true;
    })
    .start();
};

Creature.prototype.funnyCreatures = function () {
  this.activateOnFunny();
  this.isFunnyCreature = true;
  tweenEmissive('mouth', this.mouth.mesh.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('leftEye', this.eyes.left.eyeball.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('rightEye', this.eyes.right.eyeball.material, 1, 500 + this.wakeRand, 'in');
  ...
  new TWEEN.Tween(this.position).to({
    ...
    .onComplete(function () {
      self.tweenToFunnyPosition = true;
    })
    .start();
};

Creature.prototype.activateOnFunny = function () {
  this.isActive = true;
