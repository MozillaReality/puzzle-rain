...
Creature.prototype.update = function (delta, time) {
  ...
  if (this.track) {
    this.mouth.scale.y = THREE.Math.mapLinear(this.track.averageAnalyser * Math.min(1, this.track.getVolume()), 0, 100, 1, 35);
  }
...
Creature.prototype.audioLoaded = function (id) {
  if (id === this.idAudio) {
    ...
    Events.on('startEnd', this.startEnd.bind(this));
  }
};
...
Creature.prototype.startEnd = function () {
  this.isEnding = true;
  this.activateEnding();
  tweenEmissive('mouth', this.mouth.mesh.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('leftEye', this.eyes.left.eyeball.material, 1, 500 + this.wakeRand, 'in');
