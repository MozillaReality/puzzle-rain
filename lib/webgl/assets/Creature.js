...
Creature.prototype.updateAtEnding = function (delta, time) {
  this.mouth.scale.y = 8;
  this.mouth.scale.x = 0.5;
  this.updateGlow();
  var sunPos = new THREE.Vector3().setFromMatrixPosition(this.sun.matrixWorld).sub(this.parent.position);
  this.body.lookAt(sunPos);
  if (this.tweenToEndPosition) {
    this.position.y = 1.2 + (Math.sin((time * this.randExcited)) * 0.1);
  }
};

Creature.prototype.updateGlow = function () {
...
Creature.prototype.addRing = function () {
  var geometryCircleLine = new THREE.CircleGeometry(1, 8);
...
Creature.prototype.addEmitter = function () {
  this.emitter = this.particleSystem.createParticleEmitter(new THREE.TextureLoader().load('textures/magicOver.png'));
  ...
  this.emitter.setParameters({
    ...
    function (index, parameters) {
      ...
      var angle = 2 * Math.PI * (index / totalParticles);
      ...
      var acc = new THREE.Vector3(-0.05, 0, -0.05).multiply(position);
  ...
  for (var ii = 0; ii < MAX_POOFS; ++ii) {
    this.g_poofs[ii] = this.emitter.createOneShot();
  }
  this.add(this.emitter);
};

Creature.prototype.triggerPoof = function () {
  ...
  var posAbs = new THREE.Vector3().setFromMatrixPosition(this.matrixWorld);
  this.g_poofs[this.g_poofIndex].trigger([posAbs.x, posAbs.y, posAbs.z]);
  this.g_poofIndex++;
  if (this.g_poofIndex == MAX_POOFS) {
    this.g_poofIndex = 0;
  }
};
...
Creature.prototype.onEnding = function () {
  this.isEnding = true;
  this.activateEnding();
  tweenEmissive('mouth', this.mouth.mesh.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('leftEye', this.eyes.left.eyeball.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('rightEye', this.eyes.right.eyeball.material, 1, 500 + this.wakeRand, 'in');
  tweenEmissive('bodyMesh', this.bodyMesh.material, 1, 2000 + this.wakeRand, 'in');
  new TWEEN.Tween({volume: this.track.getVolume()}).to({
    volume: 0.5
  }, 2000)
    .start();
...
Creature.prototype.activateEnding = function () {
  this.isActive = true;
...
Creature.prototype.setVolumeRing = function () {
  var index = Math.floor(THREE.Math.mapLinear(Math.min(1, this.track.getVolume()), 0, 0.9, 0, 7));
  this.ring.material.map.offset.set(1 / 8 * index, 0);
// this.ring.material.linewidth = 1 + index * 5;
};
