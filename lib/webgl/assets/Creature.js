...
function Creature (race, index, myPos, myScale) {
  ...
  this.dotPos = new THREE.Vector3(this.pos.x * 1.15, 1.8 + Math.random() * 0.3, this.pos.z * 1.15);
...
Creature.prototype.addFace = function () {
...
Creature.prototype.addAudio = function () {
  ...
  this.bounceTrack = new AudioManager('effects/bounce', true, this, false, false);
};
...
Creature.prototype.addBody = function () {
  ...
  this.body.addEventListener('collide', function (e) {
    self.bounceTrack.play();
  // console.log(e.body.name);
  // if (!self.hasCollide) {
  //   self.hasCollide = true;
  //   // if (e.body.name !== 'headset' && e.body.name !== 'ball') {
  //   if (e.body.name == 'handR' || e.body.name == 'handL') {
  //
  //   }
  // }
  });
...
Creature.prototype.updatePosition = function () {
  ...
  } else if (this.handInfluencer !== '' && State.get('gamepad' + this.handInfluencer).lastPressed && !this.lockToGrab) {
    this.closeToHand();
  } else {
    this.awayToHand();
  }
...
Creature.prototype.closeToHand = function () {
  ...
  if (dist < 0.1) {
    this.startGrab(this.handInfluencer);
  } else {
    if (!this.isPlaced) {
      this.increaseAwake();
    }
    var posToGamepad = getPointInBetweenByPerc(State.get('gamepad' + this.handInfluencer).position, this.position, (0.99 - (this.awakeLevel / 20)));
...
Creature.prototype.updatePlacing = function () {
  if (this.handGrabbed !== '') {
    var isXAligned = this.getAlignedX();
    var isYAligned = this.getAlignedY();
    var isZAligned = this.getAlignedZ();
  }
...
Creature.prototype.getAlignedX = function () {
  if (this.camera.position.x > this.position.x && this.position.x > this.dotPos.x) {
    return true;
  }else if (this.camera.position.x < this.position.x && this.position.x < this.dotPos.x) {
    return true;
  }else if (this.position.x === this.camera.position.x === this.dotPos.x) {
    return true;
  } else {
    return false;
  }
...
Creature.prototype.getAlignedY = function () {
  if (this.camera.position.y > this.position.y && this.position.y > this.dotPos.y) {
    return true;
  }else if (this.camera.position.y < this.position.y && this.position.y < this.dotPos.y) {
    return true;
  }else if (this.position.y === this.camera.position.y === this.dotPos.y) {
    return true;
  } else {
    return false;
  }
...
Creature.prototype.getAlignedZ = function () {
  if (this.camera.position.z > this.position.z && this.position.z > this.dotPos.z) {
    return true;
  }else if (this.camera.position.z < this.position.z && this.position.z < this.dotPos.z) {
    return true;
  }else if (this.position.z === this.camera.position.z === this.dotPos.z) {
    return true;
  } else {
    return false;
  }
