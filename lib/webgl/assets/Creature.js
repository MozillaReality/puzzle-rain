...
function Creature (raceObj, index, myPos, myScale, minHeight, maxHeight) {
  this.index = index;
  ...
  // over / out
  this.overStatus = 'out';
  this.idAudio = this.raceObj.race + '_' + index;
...
Creature.prototype.update = function (delta, time) {
  // this.body.scale.y = THREE.Math.mapLinear(this.raceObj.track.averageAnalyser * this.raceObj.awake.awakeLevel, 0, 100, 1, 1.2);
  ...
  var glowMaterial = new THREE.ShaderMaterial(
    {
      uniforms: {
        ...
        glowColor: { type: 'c', value: new THREE.Color(MathUtils.blendColors(settings[this.raceObj.race + 'Color'], 0xffffff, 0.25)) },
...
Creature.prototype.rollOver = function () {
  if (this.overStatus !== 'over') {
    ...
    this.body.material.opacity = 0.9;
  }
};
