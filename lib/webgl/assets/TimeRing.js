...
function TimeRing () {
  ...
  this.track = new AudioManager('effects/countdown', true, this, false, false);
  this.trackStarted = false;

  this.solidRemoved = false;
  this.active = true;
  this.loopCounter = 4;
...
TimeRing.prototype.setTimeRing = function () {
  ...
  if (index !== this.ringIndex) {
    if (GameplayManager.timeToEnding - GameplayManager.experienceCounter < 9 && !this.trackStarted) {
...
TimeRing.prototype.ringSolidLoop = function () {
  this.loopCounter--;
  var self = this;
  this.ringSolid.position.y = 0.2;
  this.ringSolid.material.opacity = 0.5;
  ...
  new TWEEN.Tween(this.ringSolid.material).to({
    ...
    .easing(TWEEN.Easing.Cubic.Out)
    .onComplete(function () {
      if (State.get('endMode') === 1 && !self.solidRemoved && self.loopCounter > 0) {
        self.ringSolidLoop();
      }
    })
    .start();
...
TimeRing.prototype.endCreditsStarted = function () {
  this.solidRemoved = true;
};
