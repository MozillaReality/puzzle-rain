...
function Gamepad (hand) {
  ...
  this.mesh.load('models/hand' + this.side + '.json', function () {
    ...
    self.myBoundingPlane = new THREE.Mesh(new THREE.PlaneGeometry(boxX * 0.7, boxZ * 0.7), new THREE.MeshBasicMaterial({ color: 0xffff00, wireframe: true, transparent: true, opacity: 0 }));
...
Gamepad.prototype.dropHandler = function () {
  ...
  Events.emit('dropped', this.handlerNear);
};
