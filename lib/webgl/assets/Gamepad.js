...
function Gamepad (hand) {
  ...
  this.track = new AudioManager('effects/catch', true, this, false, false);
...
Gamepad.prototype.addBody = function () {
  var box = new THREE.Box3().setFromObject(this.mesh);
  var shape = new CANNON.Box(new CANNON.Vec3(box.size().x / 2, box.size().y / 2, box.size().z / 2));
  this.body = new CANNON.Body({
    mass: 0
  });
  this.body.addShape(shape);
  this.body.name = 'hand' + this.side;
  var self = this;
  this.body.addEventListener('collide', function (e) {
    if (e.body.name === 'ball') {
      self.track.play();
      if (vrDisplay) {
        if ('vibrate' in self.gamepadVive) {
          self.gamepadVive.vibrate([10, 100, 15, 100, 20, 100, 25, 100, 30, 100]);
        }
      }
    }
  });
