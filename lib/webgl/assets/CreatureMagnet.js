...
function Creature (race, index, myPos, myScale) {
  ...
  this.magicRActive = false;
  this.magicLActive = false;
...
Creature.prototype.updateCommon = function (delta, time) {
  ...
  this.track.setVolume(1 - Math.min(this.magnetR, this.magnetL));

  // if (this.race.status === 'awake') {
  if (Math.min(this.magnetR, this.magnetL) < 0.8) {
...
Creature.prototype.getPositionWithMagnet = function () {
  if (State.get('gamepadR') && State.get('gamepadL')) {
    ...
    var overHandPos = new THREE.Vector3(State.get('gamepadR').position.x, State.get('gamepadR').position.y + this.radius, State.get('gamepadR').position.z);
    var posToGamepad = getPointInBetweenByPerc(overHandPos, this.pos, this.magnetR);
    if (this.magnetR > this.magnetL) {
      activeMagnet = this.magnetL;
      overHandPos = new THREE.Vector3(State.get('gamepadL').position.x, State.get('gamepadL').position.y + this.radius, State.get('gamepadL').position.z);
      posToGamepad = getPointInBetweenByPerc(overHandPos, this.pos, this.magnetL);
      if (!State.get('gamepadL').hasPowerful) {
        this.magnetL += 0.02;
        this.magnetL = THREE.Math.clamp(this.magnetL, 0, 1);
      }
    } else {
      if (!State.get('gamepadR').hasPowerful) {
        this.magnetR += 0.02;
        this.magnetR = THREE.Math.clamp(this.magnetR, 0, 1);
      }
    }
...
Creature.prototype.magicOff = function (side) {
  if (side === 'R') {
    this.magicRActive = false;
  } else {
    this.magicLActive = false;
...
Creature.prototype.magicDispatched = function (side, race) {
  ...
  if (this.isActiveByHandR && side === 'R') {
    this.magicRActive = true;
    this.magnetR -= 0.01;
    this.magnetR = THREE.Math.clamp(this.magnetR, 0, 1);
  }
  if (this.isActiveByHandL && side === 'L') {
    this.magicLActive = true;
    this.magnetL -= 0.01;
    this.magnetL = THREE.Math.clamp(this.magnetL, 0, 1);
  }
