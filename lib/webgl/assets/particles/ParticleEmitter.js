...
ParticleEmitter.prototype.createParticles_ = function (firstParticleIndex, numParticles, parameters, opt_perParticleParamSetter) {
  ...
  for (var ii = 0; ii < numParticles; ++ii) {
    ...
    var angle = parameters.rotationY;
    pPosition.applyAxisAngle(axis, angle);
  ...
  this.material.uniforms.opacity.value = parameters.opacity;
 ...
 */
var dope = false;
ParticleEmitter.prototype.draw = function (camera, world, viewProjection, timeOffset) {
  this.camera = camera;
  ...
  uniforms.viewInverse.value = this.camera.matrixWorld;
