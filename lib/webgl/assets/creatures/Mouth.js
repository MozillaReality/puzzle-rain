...
function Mouth () {
  ...
  this.mesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(1, 0.5), material);
  this.add(this.mesh);
}
...
Mouth.prototype.changeMouth = function (index) {
  if (index !== this.actualMouth) {
    var row = Math.ceil(index / mouthsPerRow) - 1;
    var column = (index - 1) % mouthsPerRow;
    this.mesh.material.map.offset.x = 1 / mouthsPerRow * column;
    this.mesh.material.map.offset.y = 1 / mouthsPerColumn * (mouthsPerColumn - (row + 1));
    this.actualMouth = index;
  }
};
