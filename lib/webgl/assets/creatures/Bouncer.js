...
Bouncer.prototype.update = function (delta, time) {
  ...
  if (this.raceObj.status === 'asleep') {
    ...
    if (this.status === 'upset') {
      ...
      this.position.y = 0.5 * EasingFunctions.InOutQuad(sleepingPerc) + Math.sin(emotion.frequency * offsetTime + this.phase) * emotion.amplitude;
    ...
    this.position.y =  0.5 + Math.sin(emotion.frequency * time + this.phase) * emotion.amplitude;
