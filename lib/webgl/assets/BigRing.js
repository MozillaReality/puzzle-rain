...
BigRing.prototype.addDots = function () {
  ...
  for (var i = 0;i < this.allCreatures.length; i++) {
    ...
    dot.position.set(absPos.x * 1.2, 1.5 + Math.random() * 0.5, absPos.z * 1.2);
...
BigRing.prototype.placed = function (obj, pos) {
  ...
  if (indexPlaced !== -1) {
    ...
    if (prevIsPlaced) {
      var absPos2 = new THREE.Vector3().setFromMatrixPosition(prevObj.creature.matrixWorld);
      this.drawLine(pos, absPos2);
    }
    if (nextIsPlaced) {
      var absPos2 = new THREE.Vector3().setFromMatrixPosition(nextObj.creature.matrixWorld);
      this.drawLine(pos, absPos2);
    }
  }
};
