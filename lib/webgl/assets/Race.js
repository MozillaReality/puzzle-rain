...
Race.prototype.semiShowEmitter = function () {
  if (this.emitter) {
    this.semiShowTween = new TWEEN.Tween(this.emitter.material.uniforms.opacity).to({
      value: 0.5
    }, 500)
      .start();
  }
...
Race.prototype.semiHideEmitter = function () {
  if (this.emitter) {
    this.semiHideTween = new TWEEN.Tween(this.emitter.material.uniforms.opacity).to({
      value: 0
    }, 500)
      .start();
  }
...
Race.prototype.activeRaceChanged = function (side, race) {
  if (State.get('stage') !== 'experience' || this.status === 'awake') {
    return;
  }
  if (race === this && !this.isActiveByHandR && !this.isActiveByHandR && this.status === 'asleep') {
    if (side === 'R') {
      this.isActiveByHandR = true;
    } else {
      this.isActiveByHandL = true;
    }
    this.semiShowEmitter();
  }
