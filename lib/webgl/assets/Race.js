...
function update (delta) {
  ...
  if (settings.debugRaceStatus) {
    this.arrive.view.position.x = vector.x;
    this.arrive.view.position.y = vector.y;
    this.arrive.view.position.z = vector.z;
  }
  getStatus(this);
}
...
Race.prototype.activeRaceChanged = function (side, race) {
  if (race === this.race) {
    this.arrive.view.scale.set(2, 2, 2);
  } else {
    this.arrive.view.scale.set(1, 1, 1);
  }
};
