...
if (settings.trailerMode) {
  ...
  VREffect = require('./effects/VREffectTrailer');
} else {
...
function App () {
  ...
  function init (spectatorMode) {
    ...
    State.add('effect', effect);

    // CONTROLS
    if (navigator.vr) {
      if (navigator.getVRDisplays) {
        console.warn('Detected `navigator.vr` API, but falling back to `navigator.getVRDisplays` ...');
      } else {
        console.warn('Detected `navigator.vr` API, but `navigator.getVRDisplays` not found; cannot proceed ...');
      }
    }
    ...
    if (navigator.getVRDisplays) {
      ...
      function onFullScreenChange () {
        // Was presenting in VR but no fullscreen element? Exit fullscreen.
        if (State.get('isPresenting') && !utils.getFullscreenElement()) {
          return exitVR();
        }
      ...
      window.addEventListener('vrdisplaypresentchange', function () {
        if (!effect) {
          console.warn('Could not find the `THREE.VREffect`');
          return;
        }
        var enterVREl = document.querySelector('#enter-vr');
        if (!enterVREl) {
          console.warn('Could not find the `#enter-vr` element');
          return;
        }
        if (effect.isPresenting) {
          console.log('Started presenting in VR mode; hiding "Enter VR" screen ...');
          enterVREl.classList.add('hidden');
        } else {
          console.log('Exited VR mode; showing "Enter VR" screen ...');
          enterVREl.classList.remove('hidden');
        }
        State.add('isPresenting', !!effect.isPresenting);
      });

      window.addEventListener('dblclick', function () {
        console.log('Captured double click event to enter VR ...');
        enterVR();
      });
      ...
      return navigator.getVRDisplays().then(function (displays) {
        ...
        console.log('VR display detected: %s', vrDisplay.displayName);
  ...
  function lastAssetIsLoaded () {
    ...
    State.add('isReadyToStart', true);
    State.add('assetsLoaded', true);
    console.log('Successfully loaded all assets');
    var vrDisplay = State.get('vrDisplay');
    if (vrDisplay) {
      console.log('Using VR display: %s', vrDisplay.displayName);
      if (isModeSelected) {
        State.add('vrDisplayIsReady', true);
        State.add('vrModeIsSelected', isModeSelected);
        enterVR();
        var enterVREl = document.querySelector('#enter-vr');
        if (!enterVREl) {
          return;
        }
        enterVREl.addEventListener('click', enterVR);
        if (!State.get('isPresenting')) {
          enterVREl.classList.remove('hidden');
        }
      }
    } else {
  ...
  function exitVR () {
    ...
    enterFullscreen();
    var effect = State.get('effect');
    if (!State.get('isPresenting')) {
      return 'Promise' in window ? Promise.resolve(false) : false;
    }
    return effect.exitPresent().then(function () {
      scene.visible = false;
    }, function (err) {
      if (err) {
        console.warn(err);
  ...
  function enterFullscreen () {
    ...
    return utils.requestFullscreen(State.get('canvas') || document.documentElement);
  }
  ...
  function exitFullscreen () {
    ...
    return utils.exitFullscreen(State.get('canvas') || document.documentElement);
  }
  ...
  function enterVR () {
    ...
    return effect.requestPresent().then(function () {
      scene.visible = true;
    }, function (err) {
      if (err) {
        console.warn(err);
  ...
  function cameraSwitched (i) {
    ...
    headset.visible = cameraSpectator === camera;
  ...
  function vrFallback () {
    var canvas = settings.spectatorMode ? rendererSpectator.domElement : renderer.domElement;
    controls = new THREE.OrbitControls(camera, canvas);
    State.add('canvas', canvas);
    State.add('keysEnabled', false);
  ...
  function changeSpectatorMode () {
    ...
    console.log('Changed spectator mode to "%s"', modeSpectator);
  }
  ...
  return {
    ...
    enterVR: enterVR,
    isReadyToStart: self.isReadyToStart
  };
}
