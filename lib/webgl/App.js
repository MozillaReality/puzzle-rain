...
if (settings.trailerMode) {
  ...
  var cameraSpectator = new THREE.PerspectiveCamera(90, window.innerWidth / window.innerHeight, 0.05, 1000000);
  var Headset = require('./assets/Headset');
  var headset;
} else {
...
function App () {
  ...
  function addObjects () {
    ...
    if (settings.trailerMode) {
      cameraSpectator.position.set(-1.5, 2.1, 1.5);
      headset = new Headset();
      camera.add(headset);
      scene.add(headset);
    }
  ...
  function vrFallback () {
    if (settings.trailerMode) {
      controls = new THREE.OrbitControls(cameraSpectator, rendererSpectator.domElement);
    } else {
