...
THREE.OrbitControls = function ( object, domElement ) {
	...
	this.update = function() {
		...
		return function () {
			...
			spherical.theta = Math.max( self.minAzimuthAngle, Math.min( self.maxAzimuthAngle, spherical.theta ) );
			...
			if ( zoomChanged ||
				lastPosition.distanceToSquared( self.object.position ) > EPS ||
	...
	this.dispose = function() {
		...
		self.domElement.removeEventListener( 'contextmenu', onContextMenu, false );
		self.domElement.removeEventListener( 'mousedown', onMouseDown, false );
		self.domElement.removeEventListener( 'mousewheel', onMouseWheel, false );
		self.domElement.removeEventListener( 'MozMousePixelScroll', onMouseWheel, false ); // firefox

		self.domElement.removeEventListener( 'touchstart', onTouchStart, false );
		self.domElement.removeEventListener( 'touchend', onTouchEnd, false );
		self.domElement.removeEventListener( 'touchmove', onTouchMove, false );
		...
		//self.dispatchEvent( { type: 'dispose' } ); // should this be added here?
	...
	var pan = function() {
		...
		return function( deltaX, deltaY ) {
			...
			if ( self.object instanceof THREE.PerspectiveCamera ) {
				...
				panLeft( 2 * deltaX * targetDistance / element.clientHeight, self.object.matrix );
				panUp( 2 * deltaY * targetDistance / element.clientHeight, self.object.matrix );

			} else if ( self.object instanceof THREE.OrthographicCamera ) {

				// orthographic
				panLeft( deltaX * ( self.object.right - self.object.left ) / self.object.zoom / element.clientWidth, self.object.matrix );
				panUp( deltaY * ( self.object.top - self.object.bottom ) / self.object.zoom / element.clientHeight, self.object.matrix );
	...
	function handleMouseMoveRotate( event ) {
		...
		rotateLeft( 2 * Math.PI * rotateDelta.x / element.clientWidth * self.rotateSpeed );

		// rotating up and down along whole screen attempts to go 360, but limited to 180
		rotateUp( 2 * Math.PI * rotateDelta.y / element.clientHeight * self.rotateSpeed );
	...
	function onMouseWheel( event ) {
		...
		if ( self.enabled === false || self.enableZoom === false || state !== STATE.NONE ) return;
		...
		self.dispatchEvent( startEvent ); // not sure why these are here...
		self.dispatchEvent( endEvent );
...
THREE.OrbitControls.prototype.varructor = THREE.OrbitControls;
...
Object.defineProperties( THREE.OrbitControls.prototype, {
	...
	staticMoving : {
		...
		get: function () {
			...
			return ! this.varraint.enableDamping;
		...
		set: function ( value ) {
			...
			console.warn( 'THREE.OrbitControls: .staticMoving has been deprecated. Use .enableDamping instead.' );
			this.varraint.enableDamping = ! value;
	...
	dynamicDampingFactor : {
		...
		get: function () {
			...
			return this.varraint.dampingFactor;
		...
		set: function ( value ) {
			...
			console.warn( 'THREE.OrbitControls: .dynamicDampingFactor has been renamed. Use .dampingFactor instead.' );
			this.varraint.dampingFactor = value;
