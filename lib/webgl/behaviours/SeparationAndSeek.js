...
function SeparationAndSeek (obj, dimensions, godToFollow) {
  ...
  this.obj = obj;
  this.secureDistanceToGod = 0.04;
  this.setMax('asleep');
  this.acceleration = new THREE.Vector3();
  this.velocity = new THREE.Vector3();
  this.separateFactor = 8;
  this.seekFactor = 6;
  this.desiredSeparation = this.obj.scale.x;
...
SeparationAndSeek.prototype.raceStatusChanged = function (race, status) {
  if (this.obj.raceObj.race === race) {
    this.setMax(status);
  }
};

SeparationAndSeek.prototype.setMax = function (status) {
  switch (status) {
    case 'asleep':
      this.maxSpeed = 0.0005;
      this.maxForce = 0.00002;
      break;
    case 'awake':
      this.maxSpeed = 0.005;
