...
function SeparationAndSeek (obj, dimensions) {
  this.dimensions = 3;
  if (dimensions) {
  ...
  this.seekFactor = 6;
  this.desiredSeparation = 0.8;
  Events.on('gamepadRAdded', this.gamepadRAdded.bind(this));
  Events.on('updateScene', this.update.bind(this));
}

SeparationAndSeek.prototype.gamepadRAdded = function () {
...
SeparationAndSeek.prototype.separate = function (vehicles) {
  ...
  for ( var i = 0; i < vehicles.length; i++) {
    var myPos2D = new THREE.Vector3(this.obj.position.x, 0, this.obj.position.z);
    var posY = 0;
    if (this.dimensions === 3) {
      posY = vehicles[ i ].position.y;
    }
    var vehiclePos2D = new THREE.Vector3(vehicles[ i ].position.x, posY, vehicles[ i ].position.z);
...
SeparationAndSeek.prototype.seek = function (godPosition) {
  ...
  var myPos2D = new THREE.Vector3(this.obj.position.x, posY, this.obj.position.z);
...
SeparationAndSeek.prototype.update = function (timestamp) {
  ...
  this.applyBehaviours(this.obj.children);
  // Update velocity
