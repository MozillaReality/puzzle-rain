...
Collider.prototype.update = function () {
  this.getHandInstersctions();
  this.getHappyInstersctions();
  // this.getUpsetInstersctions();

};

Collider.prototype.getHandInstersctions = function () {
    ...
    if (collisionResults.length > 0 && collisionResults[0].distance < directionVector.length()) {
      ...
      for (var i = 0;i < collisionResults.length;i++) {
        if (collisionResults[i].distance < 0.1) {
          ...
          collisionResults[i].object.parent.makeCollision(collisionResults[i].point);
...
Collider.prototype.getHappyInstersctions = function () {
  ...
  if (happyIntersects.length > 0) {
    ...
    for (var i = 0;i < happyIntersects.length;i++) {
      ...
      if (happyIntersects[i].distance < 0.2) {
        happyIntersects[i].object.parent.makeHappy();
      }
    }
  }
};
Collider.prototype.getUpsetInstersctions = function () {
  var axis = new THREE.Vector3(1, 0, 0);
  var angle = this.obj.rotation.x;
  ...
  if (upsetIntersects.length > 0) {
    ...
    for (var i = 0;i < upsetIntersects.length;i++) {
      // console.log(upsetIntersects[i].distance);
