'use strict';

var THREE = require('three');
var TWEEN = require('tween.js');

var Events = require('../../events/Events');
var State = require('../../state/State');
var settings = require('../../settings');

var limitAwake = 1.5;

function Awake (obj) {
  this.delta = 0;
  this.raceToAwake = obj;
  // this.raceToAwake.status = 'asleep' (awakeLevel from 0 to 0.5)
  // this.raceToAwake.status = 'awake' (awakeLevel from 0.5 to 1)

  this.isEnding = false;

  this.awakeLevel = 0;

  Events.on('magicDispatched', this.magicDispatched.bind(this));
  Events.on('updateScene', this.update.bind(this));
  Events.on('onEnding', this.onEnding.bind(this));
}

Awake.prototype.update = function (delta, time) {
  if (this.isEnding) {
    return;
  }
  this.delta = delta;
  this.getStatus();
  if (this.awakeLevel > 0) {
    // this.awakeLevel = THREE.Math.clamp(this.awakeLevel - this.delta / 48, 0, limitAwake);
  }
};

Awake.prototype.magicDispatched = function (side, race) {
  if (this.raceToAwake === race) {
    var awakeVal = 3;
    if (!State.get('vrDisplay')) {
      // To awake earlier
      awakeVal = 2;
    }

    this.awakeLevel = THREE.Math.clamp(this.awakeLevel + this.delta / awakeVal, 0, limitAwake);
    // var avgArrivePosY = THREE.Math.clamp(this.raceToAwake.arrive.position.y, 0, 2);
    // console.log(avgArrivePosY);
    // this.awakeLevel = avgArrivePosY;

  }
};

Awake.prototype.getStatus = function () {
  var currentStatus = 'asleep';
  if (this.awakeLevel > 0.5) {
    currentStatus = 'awake';
  }
  if (this.raceToAwake.status !== currentStatus) {
    if (this.raceToAwake.status === 'asleep') {
      var tween = new TWEEN.Tween(this).to({
        awakeLevel: 1
      }, 3000)
        // .easing(TWEEN.Easing.Circular.In)
        .start();
    }
    this.raceToAwake.status = currentStatus;
    Events.emit('raceStatusChanged', this.raceToAwake, this.raceToAwake.status);
  }
};

Awake.prototype.onEnding = function () {
  this.isEnding = true;
};

module.exports = Awake;
