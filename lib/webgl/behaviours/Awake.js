...
Awake.prototype.update = function (delta, time) {
  ...
  if (this.awakeLevel > 0) {
    this.awakeLevel = THREE.Math.clamp(this.awakeLevel - this.timestamp / 2, 0, limitAwake);
  }
};

Awake.prototype.magicDispatched = function (side, race) {
  if (this.obj.race === race) {
    this.awakeLevel = THREE.Math.clamp(this.awakeLevel + this.timestamp, 0, limitAwake);
  }
};
