...
function Talking (obj) {
  ...
  this.speechArrIndex = 0;
  this.speechArr = [];

  setInterval(this.speak.bind(this), 80);
...
Talking.prototype.speak = function () {
  if (this.obj.raceObj.status === 'asleep') {
    ...
    if (this.speechArr.length > 0) {
      if (this.speechArrIndex < this.speechArr.length) {
        this.renderMouth(this.speechArr[ this.speechArrIndex ]);
        this.speechArrIndex++;
      } else {
        this.speechArrIndex = 0;
      }
